...
const BookCard: React.FC<BookCardProps> = ({ book, onAddToCart }) => {
  ...
  return (
    <div className="bg-white rounded-3xl shadow-2xl overflow-hidden transform hover:-translate-y-3 transition-transform duration-300">
      {/* Book Cover */}
      ...
      <div className="p-5">
        <Link href={`/book/${book.id}`} className="block cursor-pointer">
          <h3 className="text-xl font-semibold text-gray-900 truncate hover:text-purple-600 transition-colors duration-200">{book.title}</h3>
          <p className="text-base text-gray-700 mt-1">by {book.author}</p>
        </Link>
        ...
        <div className="mt-3">
          {book.genre.slice(0, 2).map((g) => (
            <span key={g} className="inline-block bg-indigo-200 rounded-full px-3 py-1 text-xs font-semibold text-gray-800 mr-2 mb-2">
              {g}
            </span>
          ))}
          {book.genre.length > 2 && (
